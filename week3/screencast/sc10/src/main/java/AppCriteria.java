        ...
        try {
            {
                ...
                List<Airline> airlines = em.createQuery(cq).getResultList();
            ...
            log.debug("Search for French Airlines going to Miami");
            {
                ...
                cq.select(rt.get("airline")).distinct(true)
                        .where(cb.and(cb.equal(al.get("country"), "France"), cb.equal(ap.get("city"), "Miami")));
            ...
            log.debug("Search Number of airlines from CDG");
            {
                ...
                cq.select(cb.countDistinct(rt.get("airline"))).where(cb.equal(ap.get("iata"), "CDG"));
            ...
            log.debug("Search Best airlines with number of dest airports to USA from CDG, ordered by 'coverage'");
            {
                ...
                cq.select(cb.construct(AirlineCoverage.class, cnt, apD.get("country"), al.get("name")))
                        .where(cb.and(cb.equal(apD.get("country"), "United States")), cb.equal(apS.get("iata"), "CDG"))
                        .groupBy(apD.get("country"), al.get("name")).orderBy(cb.desc(cnt));

                List<AirlineCoverage> res = em.createQuery(cq).getResultList();
