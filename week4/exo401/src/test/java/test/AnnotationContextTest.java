...
public class AnnotationContextTest {
    ...
    public void testComponentAnnotations() {
        ...
        for (Class<?> k : Arrays.asList(ChangePwdApp.class, UserJPADao.class, HmacChecker.class, MD5Checker.class))
            Assert.assertNotNull("Class " + k.getName() + " should be a Component", k.getAnnotation(Component.class));
    }

    @Test
    public void testSpringConfig() {
        Class<?> k = SpringConfig.class;
        Assert.assertNotNull("1", k.getAnnotation(Configuration.class));
        Assert.assertNotNull("2", k.getAnnotation(ComponentScan.class));
        List<String> scan = Arrays.asList(k.getAnnotation(ComponentScan.class).basePackages());
        Assert.assertTrue("3", scan.contains("fr.eservices.drive.dao.impl"));
        Assert.assertTrue("4", scan.contains("fr.eservices.drive.util"));
        Assert.assertTrue("5", scan.contains("fr.eservices.drive.app"));
        ...
        Assert.assertTrue("6", exposedBean.contains("EntityManager"));
    }
    ...
    public void testInjections() {
        Assert.assertTrue("1", getFieldTypeAnnotated(ChangePwdApp.class, Autowired.class).contains("IUserDao"));
        Assert.assertTrue("2", getFieldTypeAnnotated(UserJPADao.class, Autowired.class).contains("EntityManager"));
        Assert.assertTrue("3", getFieldTypeAnnotated(UserDao.class, Autowired.class).contains("PasswordChecker"));
    }
